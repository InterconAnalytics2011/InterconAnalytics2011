...
def Error(reason='internal_error', message='Internal Error', detailed_message=None , tqx=""):
  '''
  Returns Error code
  '''
  tqx_dict = {}
  ...
  if tqx_dict.get("out", "json") == "json":
    response_handler = tqx_dict.get("responseHandler",
  			      "google.visualization.Query.setResponse")
    return ("%s({'version':'0.6', 'reqId':'%s', 'status':'error', 'errors':[{'reason':'%s','message':'%s'}]});") % (response_handler, tqx_dict.get("reqId", 0), reason, message)
  elif tqx_dict["out"] == "html":
    return ('Error (%s): %s') % (reason,message)
  elif tqx_dict["out"] == "csv":
    return ('Error (%s): %s') % (reason,message)
  elif tqx_dict["out"] == "tsv-excel":
    return ('Error (%s): %s') % (reason,message)
  else:
    raise DataTableException("'out' parameter: '%s' is not supported" % tqx_dict["out"])
